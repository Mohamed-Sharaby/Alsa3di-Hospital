                    ...
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            ...
                            <a href="{{route('admin.specializations.create')}}"
                               class="btn btn-purple btn-rounded w-md waves-effect waves-light m-b-5">اضافة تخصص
                                جديد</a>
                            <br>
                            <br>
                            ...
                            <table id="datatable-buttons" class="table table-striped table-bordered text-center">
                                ...
                                @foreach($items as $index => $item)
                                    <tr>
                                        ...
                                        <td>{{$item->ar_name}}</td>
                                        <td>
                                            @include('dashboard.specializations.specialization-details')
                                        </td>
                                        <td>
                                            @if($item->image)
                                        ...
                                        <td class="text-center">
                                            <form
                                                action="{{ route('admin.active', ['id' => $item->id, 'type' => 'Specialization']) }}"
                                            ...
                                            <a href="{{route('admin.specializations.edit',$item->id)}}"
                                               class="btn-icon waves-effect btn btn-primary btn-sm ml-2 rounded-circle"><i
                                                    class="fa fa-edit"></i></a>


                                            <button data-url="{{route('admin.specializations.destroy',$item->id)}}"
                                                    data-name="{{ $item->name }}"
